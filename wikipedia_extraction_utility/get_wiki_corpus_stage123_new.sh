...
while IFS= read -r line; do
	...
	## call stage3 loop with name for category = catnamefile?
	./get_wiki_corpus_stage3.sh "$curr_depth" "$article_list" "$subcat_list" "$use_depth" "$use_max" "$max_articles" "$max_depth" "$catnamefile" "$filecontent"
