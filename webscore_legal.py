...
def do_provider_search(term,provider):
    ...
    ## response = requests.get(url).text ##2025 request fix
...
def webscore_one_term(term,debug=False,use_web_score_dict=False,provider='yahoo'):
    web_count = 1
    quantity_response = requests.get(
        'https://api.case.law/v1/cases/?search=' + term,
        headers={'Authorization': '75985a5f356905ed74fa631f1a44962852bad577'}
    )

    quality_response = requests.get(
        'https://api.case.law/v1/cases/?search=' + term + "&court_id=9009",
        headers={'Authorization': '75985a5f356905ed74fa631f1a44962852bad577'}
    )
    SC_count = quality_response.json()['count']
    all_count = quantity_response.json()['count']
    if all_count == 0 or SC_count == 0:
        return (0, web_count)
    
    score = log(all_count,10)/7 * (log(SC_count,10)/log(all_count,10))
