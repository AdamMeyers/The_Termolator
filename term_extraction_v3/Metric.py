...
class Metric:
    ...
    def __init__(self, rdgDir, general, working_dir = '.'):
        # available metrics
        ...
        if type(general)== type(str()):
            ...
            gen = map(lambda x: Document(x.strip()), open(general).readlines())
        ...
        self.rdgDocs = map(lambda x: Document(x.strip()), open(rdgDir).readlines())
        logging.debug('done')
